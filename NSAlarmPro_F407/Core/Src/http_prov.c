...
#if HTTPD_USE_CUSTOM_FSDATA
const struct fsdata_file fsdata_root = {0};
#endif
static const char http_default_404[] =
    "<html><head><meta charset=\"utf-8\"><title>404</title></head><body><h1>404</h1><p>Risorsa non trovata.</p></body></html>";
static const char http_post_only_msg[] =
    "<html><head><meta charset=\"utf-8\"></head><body><p>Utilizzare POST per questa risorsa.</p></body></html>";
...
{
    HTTP_FILE_KIND_HTML,
    HTTP_FILE_KIND_STREAM,
    HTTP_FILE_KIND_GENERIC
} http_file_kind_t;

typedef struct
{
    bool in_use;
    http_file_kind_t kind;
    const char *payload;
    uint32_t payload_len;
#if LWIP_HTTPD_SUPPORT_CONTENT_TYPE
    const char *content_type;
#endif
} http_file_state_t;

#define HTTP_MAX_CONCURRENT_FILES  4

static http_file_state_t file_state_pool[HTTP_MAX_CONCURRENT_FILES];

static http_file_state_t *alloc_file_state(void)
{
    for (size_t i = 0; i < LWIP_ARRAYSIZE(file_state_pool); i++)
    {
        if (!file_state_pool[i].in_use)
        {
            file_state_pool[i].in_use = true;
            file_state_pool[i].payload = NULL;
            file_state_pool[i].payload_len = 0U;
            file_state_pool[i].kind = HTTP_FILE_KIND_GENERIC;
#if LWIP_HTTPD_SUPPORT_CONTENT_TYPE
            file_state_pool[i].content_type = NULL;
#endif
            return &file_state_pool[i];
        }
    }
    return NULL;
}

static void release_file_state(http_file_state_t *state)
{
    if (state != NULL)
    {
        state->in_use = false;
        ...
        state->kind = HTTP_FILE_KIND_GENERIC;
#if LWIP_HTTPD_SUPPORT_CONTENT_TYPE
...
err_t fs_open_custom(struct fs_file *file, const char *name)
{
    ...
    http_file_state_t *state = alloc_file_state();
    if (state == NULL)
    {
        return ERR_MEM;
    }
    ...
    {
        payload = cgi_root_handler(0, 0, NULL, NULL);
        state->kind = HTTP_FILE_KIND_HTML;
    }
    else if (strcmp(name, "/provision/stream") == 0)
    {
        payload = cgi_stream_handler(0, 0, NULL, NULL);
        state->kind = HTTP_FILE_KIND_STREAM;
        content_type = "text/event-stream";
    }
    else if (strcmp(name, "/provision") == 0)
    {
        payload = http_post_only_msg;
        state->kind = HTTP_FILE_KIND_GENERIC;
    }
    else
    {
        payload = http_default_404;
        state->kind = HTTP_FILE_KIND_GENERIC;
    }

    if (payload == NULL)
    {
        release_file_state(state);
        return ERR_VAL;
    }

    state->payload = payload;
    state->payload_len = (uint32_t)strlen(payload);
#if LWIP_HTTPD_SUPPORT_CONTENT_TYPE
    state->content_type = content_type;
#endif

    memset(file, 0, sizeof(*file));
    file->data = state->payload;
    file->len = (int)state->payload_len;
    file->index = 0;
    file->pextension = state;
#if LWIP_HTTPD_CUSTOM_FILES
    file->state = state;
#endif
#if defined(LWIP_HTTPD_SUPPORT_CONTENT_LEN) && LWIP_HTTPD_SUPPORT_CONTENT_LEN
    file->content_len = (int)state->payload_len;
#endif
#if LWIP_HTTPD_SUPPORT_CONTENT_TYPE
    file->content_type = state->content_type;
#endif
    return ERR_OK;
}
...
void fs_close_custom(struct fs_file *file)
{
    ...
    http_file_state_t *state = NULL;
    if (file->pextension != NULL)
    {
        state = (http_file_state_t *)file->pextension;
    }
#if LWIP_HTTPD_CUSTOM_FILES
    if (state == NULL && file->state != NULL)
    {
        state = (http_file_state_t *)file->state;
    }
#endif
    release_file_state(state);
}
...
int fs_read_custom(struct fs_file *file, char *buffer, int count)
{
    ...
    int to_copy = (count < remaining) ? count : remaining;
    memcpy(buffer, file->data + file->index, (size_t)to_copy);
    file->index += to_copy;
    return to_copy;
}
